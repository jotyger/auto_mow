...
PurePursuit::PurePursuit() : ld_(1.0), v_max_(1), v_(v_max_), w_max_(1.0), pos_tol_(0.1), idx_(0),
...
void PurePursuit::computeVelocities(nav_msgs::Odometry odom)
{
  ...
  try
  {
    ...
    tf_broadcaster_.sendTransform(lookahead_);          /// PROBABLE ERROR LOCATION
...
void PurePursuit::receivePath(nav_msgs::Path new_path)
{
  ...
  {
    ROS_WARN_STREAM("PROCESSING PATH IN LOOP");
    path_ = new_path;
    idx_ = 0;
