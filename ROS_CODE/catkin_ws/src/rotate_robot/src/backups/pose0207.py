...
def follow_path():
	...
	if (iteration == 0):
		...
		current_location = current_pose.position
		...
		rospy.loginfo("Current Position (After Movement): " +str(current_pose))
		...
		current_pose.orientation.z = quaternion[2]
		current_pose.orientation.w = quaternion[3]
		rospy.loginfo("Orientation (Steered): " + str(current_pose.orientation))
		...
		publish_pose = PoseStamped(header = header, pose = current_pose)
